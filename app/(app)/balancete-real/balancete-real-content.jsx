...
import { ArrowUpCircle, ArrowDownCircle, Calendar, ChevronRight, ChevronDown, TrendingUp, Filter, Building2, CheckCircle } from "lucide-react"

export function BalanceteRealContent() {
  ...
  // Load contas from API (apenas pagas e no fluxo de caixa)
  const loadContas = async () => {
    try {
      const res = await fetch("/api/contas")
      if (res.ok) {
        const data = await res.json()
        // Filtrar apenas contas pagas e no fluxo de caixa
        const contasRealizadas = (Array.isArray(data) ? data : [])
          .filter(conta => conta.pago === true && conta.noFluxoCaixa === true)
        setTodasContas(contasRealizadas)
      }
    } catch (error) {
  ...
  // Calculate totals (only realizado)
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Balancete Simplificado - Realizado"
        description="Visão dos valores realizados por centro de custo e receita"
        actions={
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-foreground">Receitas Realizadas</span>
            </div>
          </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-foreground">Despesas Realizadas</span>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-lg font-bold text-red-600">
                {formatCurrency(totalCustoRealizado)}
              </span>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-gradient-to-br from-primary/5 to-primary/10 border-primary/20">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-foreground">Resultado Realizado</span>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex items-baseline justify-between">
              <span className="text-xs text-muted-foreground">Saldo Realizado</span>
              <span className={`text-lg font-bold ${resultadoRealizado >= 0 ? "text-emerald-600" : "text-red-600"}`}>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-right py-2 px-3 text-xs font-medium text-muted-foreground uppercase">Realizado</th>
                ...
                <tbody>
                  {centrosReceita.filter(c => !c.parentId).map((centro) => {
                    ...
                    const percentual = totalReceitaRealizada > 0 ? (centro.realizado / totalReceitaRealizada) * 100 : 0
                    ...
                    return (
                      <Fragment key={centro.id}>
                        <tr className="border-b border-border hover:bg-muted/50 transition-colors">
                          ...
                          <td className="py-3 px-3 text-sm font-medium text-foreground text-right">{formatCurrency(centro.realizado)}</td>
                        ...
                        {isExpanded && subcentros.map((subcentro) => {
                          const subPercentual = totalReceitaRealizada > 0 ? (subcentro.realizado / totalReceitaRealizada) * 100 : 0
                          return (
                            <tr key={subcentro.id} className="border-b border-border hover:bg-muted/50 transition-colors bg-muted/30">
                              ...
                              <td className="py-2 px-3 text-sm font-medium text-foreground text-right">{formatCurrency(subcentro.realizado)}</td>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {centrosCusto.filter(c => !c.parentId).map((centro) => {
                    ...
                    const hasSubcentros = subcentros.length > 0
                    const isExpanded = expandedCenters.has(centro.id)
                    const percentual = totalCustoRealizado > 0 ? (centro.realizado / totalCustoRealizado) * 100 : 0
                    ...
                    return (
                      <Fragment key={centro.id}>
                        ...
                        {isExpanded && subcentros.map((subcentro) => {
                          const subPercentual = totalCustoRealizado > 0 ? (subcentro.realizado / totalCustoRealizado) * 100 : 0
                          return (
                            <tr key={subcentro.id} className="border-b border-border hover:bg-muted/50 transition-colors bg-muted/30">
                              <td className="py-2 px-2"></td>
                              <td className="py-2 px-3 text-sm text-foreground pl-8">
      ...
      <Modal
        ...
        title="Filtro por Centro de Custo/Receita - Realizado"
        description="Selecione um centro de custo ou receita para ver as contas já pagas e no fluxo de caixa"
        size="xl"
      >
          ...
          <div className="border border-border rounded-lg p-4 overflow-auto max-h-[500px]">
            {!centroSelecionado ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                ...
                <p className="text-xs text-muted-foreground/70 mt-1">para ver as contas realizadas</p>
              </div>
            ) : (
              <div>
                ...
                <div className="mb-4 pb-4 border-b border-border">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-3">
                    <div className="bg-muted/50 rounded-lg p-3">
                      <p className="text-xs text-muted-foreground mb-1">Valor Total Realizado</p>
                    ...
                    <div className="bg-muted/50 rounded-lg p-3">
                      ...
                      <p className="text-lg font-bold text-primary">
                        {formatPercentage(
                          centroSelecionado.tipo === 'despesa'
                            ? (totalCustoRealizado > 0 ? (totalContasCentro / totalCustoRealizado) * 100 : 0)
                            : (totalReceitaRealizada > 0 ? (totalContasCentro / totalReceitaRealizada) * 100 : 0)
                        )}
                      </p>
                    </div>
                  </div>
                ...
                {loadingContas ? (
                  ...
                  <div className="text-center py-8">
                    <p className="text-sm text-muted-foreground">Nenhuma conta realizada vinculada a este centro</p>
                  </div>
                ) : (
                  <div>
                    <h5 className="text-xs font-medium text-muted-foreground uppercase mb-2 flex items-center gap-2">
                      <CheckCircle className="h-3 w-3 text-emerald-600" />
                      Contas Pagas ({contasCentro.length})
                    </h5>
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-border">
                            ...
                            <tr key={conta.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                              <td className="py-2 px-2">
                                <div>
                                  ...
                                  {conta.beneficiario && (
                                    <p className="text-xs text-muted-foreground ml-4">{conta.beneficiario}</p>
                                  )}
                                </div>
                              </td>
