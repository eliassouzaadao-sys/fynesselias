  ...
  return (
    <Card className="p-5 hover:shadow-md transition-shadow">
      <div className="space-y-4">
        ...
        {/* Lista de gastos/descontos */}
        <div className="space-y-2">
          <p className="text-xs font-medium text-muted-foreground flex items-center gap-1">
            <Receipt className="h-3 w-3" />
            Gastos no período ({(socio.gastosDetalhados || socio.ultimosGastos || []).length})
          </p>
          {((socio.gastosDetalhados && socio.gastosDetalhados.length > 0) ||
            (socio.ultimosGastos && socio.ultimosGastos.length > 0)) ? (
            <div className="space-y-1 max-h-32 overflow-y-auto">
              {(socio.gastosDetalhados || socio.ultimosGastos || []).map((gasto) => (
                <div key={gasto.id} className={`flex justify-between text-xs px-2 py-1 rounded ${
                  gasto.tipo === 'previsto' ? 'bg-orange-50' : 'bg-gray-50'
                }`}>
                  <span className="text-foreground truncate max-w-[60%]">{gasto.descricao}</span>
                  <span className={`font-medium ${
                    gasto.tipo === 'previsto' ? 'text-orange-600' : 'text-red-600'
                  }`}>-{formatCurrency(gasto.valor)}</span>
                </div>
              ))}
            </div>
          ) : (
